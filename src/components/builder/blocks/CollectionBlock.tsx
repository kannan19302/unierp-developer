...
function resolveMeta(collection: any) {
  ...
  const fields: FieldMeta[] = Array.isArray(collection?.fields)
    ? collection.fields
    : [];
  const titleField =
    settings.titleField ||
    ...
    (async () => {
      ...
      try {
        if (!props.tenantSlug) {
          ...
          // Public live site.
          const q = props.tenantSlug
            ? `?tenant=${encodeURIComponent(props.tenantSlug)}`
            : "";
          const res = await fetch(
            `/api/v1/public/web/collections/${collectionSlug}${q}`,
          );
    ...
    <section style={{ padding: "72px 20px" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        {!collectionSlug ? (
          ...
          >
            <strong>Collection block</strong>
            <br />
            Bind a collection in the inspector to display content.
          </div>
        ) : loading ? (
          ...
          <div
            style={{
              ...
              gridTemplateColumns:
                layout === "list"
                  ? "1fr"
                  : `repeat(auto-fit, minmax(${Math.max(220, Math.floor(1100 / columns))}px, 1fr))`,
              gap: 28,
            }}
          >
            {shown.map((item) => {
