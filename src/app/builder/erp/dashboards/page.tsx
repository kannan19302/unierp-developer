        ...
        actions={
          <div className="ui-flex ui-gap-2">
            ...
            <button
              ...
              onClick={() => router.push("/builder/erp/dashboards/new")}
            >
              <PlusCircle size={15} />
      ...
      <div className="builder-stats-grid">
        {[
          ...
          },
          {
            ...
            value: dashboards
              .reduce(
                (a, d) => a + (Array.isArray(d.widgets) ? d.widgets.length : 0),
                0,
              )
              .toString(),
            icon: Layout,
      ...
      {loading ? (
        ...
        <div className={`ui-card ${styles.s7}`}>
          ...
          <p className="ui-text-muted">
            No dashboards yet. Create your first KPI dashboard!
          </p>
        </div>
      ) : (
        <div className={styles.s9}>
          {filtered.map((d) => (
            <div
              key={d.id}
              className={`ui-card ${styles.s10} ${styles.dashboardCard}`}
            >
              <div className={styles.s11}>
                <div className="ui-hstack-2">
                  ...
                  <div>
                    ...
                    {d.description && (
                      <p className="ui-text-xs-muted m-0">{d.description}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="ui-flex ui-gap-1">
                <button
                  className={`ui-btn ui-btn-secondary ${styles.s20}`}
                  onClick={() => router.push(`/builder/erp/dashboards/${d.id}`)}
                >
                  <Edit3 size={13} /> <span>Edit</span>
                </button>
                <button
                  className={`ui-btn ui-btn-secondary ${styles.s21}`}
                  onClick={() => router.push(`/dashboard?dashboardId=${d.id}`)}
                >
                  <Eye size={13} />
                </button>
                <button
                  className={`ui-btn ${styles.s22}`}
