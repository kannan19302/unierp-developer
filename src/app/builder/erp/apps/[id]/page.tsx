...
function EmptyState({
  ...
  return (
    <div className={styles.s4}>
      ...
      <p
        style={{ marginBottom: action ? "var(--space-4)" : 0 }}
        className={styles.s7}
      >
        {description}
      </p>
...
function AddComponentModal({
  ...
  useEffect(() => {
    ...
    const fetchItems = async () => {
      setLoading(true);
      try {
        const data = await client.get<any[] | { data?: any[] }>(
          `/builder/${ADD_COMPONENT_ENDPOINTS[type]}`,
        );
    ...
    <div className={styles.s9}>
      ...
      <div className={styles.s11}>
        <div className={styles.s12}>
          ...
          <button onClick={onClose} className={styles.s14}>
            <X size={20} />
          </button>
        </div>
        ...
        <div className={styles.s15}>
          ...
          <div className="ui-flex ui-gap-2">
            <input
              ...
              placeholder={`New ${typeLabel} name...`}
        ...
        <div className={styles.s22}>
          {loading ? (
            ...
            <div className={styles.s24}>
              No existing {typeLabel.toLowerCase()}s found. Create a new one
              above.
            </div>
          ) : (
...
function AddPageModal({
  ...
  const [type, setType] = useState<"form" | "list" | "dashboard" | "custom">(
    "form",
  );
  const [dataSource, setDataSource] = useState<"existing" | "build">(
    "existing",
  );
    ...
    <div className={styles.s9}>
      ...
      <div className={styles.s28}>
            ...
            {(["form", "list", "dashboard", "custom"] as const).map((t) => (
              <button
                ...
                style={{
                  ...
                  background:
                    type === t ? "var(--color-primary-bg)" : "var(--color-bg)",
          ...
          <div className={styles.s32}>
                  ...
                  style={{
                    ...
                    background:
                      dataSource === v
                        ...
                        : "var(--color-bg)",
                    fontWeight: dataSource === v ? 600 : 400,
                    color:
                      dataSource === v
                        ? "var(--color-primary)"
                        : "var(--color-text-secondary)",
            ...
            {dataSource === "existing" ? (
              ...
              <div className={styles.s37}>
                The form builder will open. On save, your form is linked and
                this page is created bound to it.
              </div>
            )}
          </div>
        )}
        ...
        <div className={styles.s38}>
          <button onClick={onClose} className={styles.s39}>
            Cancel
          </button>
          <button
            ...
            disabled={
              submitting ||
              !name.trim() ||
              (needsForm && dataSource === "existing" && !formId)
            }
            className={styles.s40}
          >
            {needsForm && dataSource === "build" ? (
              <>
                <Wand2 size={15} /> Build Form & Add Page
              </>
            ) : submitting ? (
              "Adding..."
            ) : (
...
function AddDataModelModal({
  ...
  const [fields, setFields] = useState<
    { name: string; type: string; required: boolean; label: string }[]
  ...
  const addField = () =>
    setFields([
      ...fields,
      { name: "", type: "Data", required: false, label: "" },
    ]);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await client.post(`/builder/modules/${appId}/data-models`, {
        name,
        fields,
      });
    } catch {
...
function OverviewSection({
  ...
  return (
    <div>
      <SectionHeader
        title="App Overview"
        subtitle="Configure your application's basic information"
      />
      ...
      <div className={styles.s57}>
        ...
        <div className={styles.s64}>
          <button onClick={handleSave} disabled={saving} className={styles.s65}>
...
function ComponentListSection({
  ...
  const components = (
    Array.isArray(app.components) ? app.components : []
  ...
  const handleRemove = async (componentId: string) => {
    try {
      await client.delete(
        `/builder/modules/${app.id}/components/${componentId}`,
      );
    } catch {
      /* ignore */
    }
  ...
  return (
    <div>
      <SectionHeader
        title={typeLabel + "s"}
        subtitle={`Manage ${typeLabel.toLowerCase()} components linked to this app`}
        action={headerAction}
      />
      {components.length === 0 ? (
        <EmptyState
          icon={Icon}
          title={`No ${typeLabel.toLowerCase()}s yet`}
          description={`Link or create ${typeLabel.toLowerCase()} components for your app.`}
          action={
            <AddButton
          ...
          {components.map((c: any) => (
            <div key={c.id} className={styles.s67}>
              ...
              <div className="ui-flex ui-items-center ui-gap-1">
                {onEditItem && (
                  <button
                    onClick={() => onEditItem(c.refId)}
                    title={`Edit ${typeLabel.toLowerCase()}`}
                    className={`${styles.s50} ${styles.editControl}`}
                  >
                    <Pencil size={15} />
                  </button>
                )}
                <button
                  onClick={() =>
                    setDeleteComponentTarget({ id: c.id, name: c.name })
                  }
                  className={`${styles.s50} ${styles.deleteControl}`}
                >
                  <Trash2 size={16} />
                </button>
              </div>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (deleteComponentTarget) {
            handleRemove(deleteComponentTarget.id);
...
function PagesSection({
  ...
  return (
    <div>
      <SectionHeader
        title="Pages"
        subtitle="Define the pages and navigation for your app"
      ...
      {pages.length === 0 ? (
        <EmptyState
          icon={Layers}
          title="No pages yet"
          description="Add pages to define how users navigate your app."
          action={
            <AddButton onClick={() => setShowAdd(true)} label="Add Page" />
          }
        />
      ) : (
        <div className="ui-stack-3">
          {pages.map((p: any) => (
            <div key={p.id} className={styles.s67}>
              ...
              <div className={styles.s72}>
                ...
                <button
                  onClick={() =>
                    setDeletePageTarget({ id: p.id, name: p.name })
                  }
                  className={styles.s50}
                >
                  <Trash2 size={16} />
                </button>
              </div>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (deletePageTarget) {
            handleRemove(deletePageTarget.id);
  ...
  return (
    <>
      <div>
        <SectionHeader
          title="Data Models"
          subtitle="Define custom data structures for your app"
          action={
            <AddButton
              onClick={() => setShowAdd(true)}
        ...
        {dataModels.length === 0 ? (
          <EmptyState
            ...
            description="Define the data structures that power your app."
            action={
              <AddButton
                onClick={() => setShowAdd(true)}
                label="Add Data Model"
                ...
                <div className={styles.s75}>
                  <div className="ui-hstack-3">
                    ...
                    <div>
                      ...
                      <div className="ui-text-xs-soft">
                        {(dm.fields || []).length} fields
                      </div>
                    </div>
                  </div>
                  <button
                    onClick={() =>
                      setDeleteModelTarget({ id: dm.id, name: dm.name })
                    }
                    className={styles.s50}
                  >
                    <Trash2 size={16} />
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (deleteModelTarget) {
            handleRemove(deleteModelTarget.id);
...
function TestEngineSection({
  ...
  const [results, setResults] = useState<any>(
    app.testResults &&
      typeof app.testResults === "object" &&
      !Array.isArray(app.testResults) &&
      (app.testResults as any).score != null
      ? app.testResults
      : null,
  );
  const client = useApiClient();
  ...
  return (
    <div>
      <SectionHeader
        title="Test Engine"
        subtitle="Validate your app before publishing"
        action={
          <button
            onClick={runTests}
            disabled={running}
          ...
          >
            {running ? (
              <>
                <RefreshCw size={16} className="spin" /> Running Tests...
              </>
            ) : (
              <>
      ...
      {results ? (
        <div>
            ...
            <div className={styles.s81}>
              ...
              <div className="flex-1">
                ...
                <div className={styles.s86}>
                  <span>
                    <strong>{results.passed}</strong> passed
                  </span>
                  <span>
                    <strong>{results.failed}</strong> failed
                  </span>
                  <span>
                    <strong>{results.totalTests}</strong> total tests
                  </span>
                </div>
                {results.lastRunAt && (
                  <div className={styles.s87}>
                    Last tested: {new Date(results.lastRunAt).toLocaleString()}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className={styles.s88}>
              ...
              <div className={styles.s90}>
                {[...results.history].reverse().map((h: any, i: number) => (
                  <div
                    key={i}
                    title={`${h.score}% · ${new Date(h.runAt).toLocaleString()}`}
                    style={{
          ...
          {/* Sandbox simulations */}
          {Array.isArray(results.simulations) &&
                ...
                <div className="ui-stack-2">
                  {results.simulations.map((s: any, i: number) => (
                    <div key={i} className={styles.s92}>
                      {s.status === "pass" ? (
                        <CheckCircle size={16} className={styles.s93} />
                      ) : (
                        <XCircle size={16} className={styles.s94} />
                      )}
                      <div className="flex-1">
          ...
          {results.errors && results.errors.length > 0 && (
            <div className="ui-stack-2">
              {results.errors.map((err: any, i: number) => {
                const sev =
                  severityConfig[err.severity] ?? severityConfig.info!;
                ...
                return (
                  ...
                  >
                    <SevIcon
                      ...
                      className={styles.s97}
                    />
                    <div>
                      <div style={{ color: sev.color }} className={styles.s98}>
                        {err.message}
                      </div>
        ...
        <EmptyState
          ...
          description="Run the test engine to validate your app's components, pages, and data integrity."
        />
      )}
    </div>
  );
...
function PreviewSection({ app }: { app: AppModule }) {
  ...
  const formForPage = (page: any) => {
    if (!page) return null;
    if (page.formId)
      return forms.find((f: any) => f.id === page.formId) || null;
  ...
  if (pages.length === 0) {
    return (
      <div>
        <SectionHeader
          title="Preview"
          subtitle="Run your app in a simulated shell before publishing"
        />
        <EmptyState
          icon={MonitorPlay}
          title="No pages to preview"
          description="Add pages to your app to preview the runtime experience."
        />
      </div>
    );
  }
  ...
  return (
    <div>
      <SectionHeader
        title="Preview"
        subtitle="A simulated runtime shell — form submissions are captured locally, not saved"
        action={
          isInstalledRoute && activePage ? (
            <a
              href={`/app/${moduleSlug}/${activePage.slug}`}
              target="_blank"
              rel="noreferrer"
        ...
        <div className={styles.s107}>
          ...
          {activePage?.type === "custom" ? (
            <div className={styles.s109}>
              {(() => {
                const layout = Array.isArray(activePage.layout)
                  ? activePage.layout
                  : [];
                if (layout.length === 0) {
                  return (
                    <div className={styles.s110}>
                      <EmptyState
                        ...
                        description="Go to the Pages tab and click Design Layout to build this page."
                      />
                    </div>
                  );
                }
                return layout.map((w: any) => (
                  <div
                    key={w.id}
                    style={{ gridColumn: `span ${w.gridSpan || 12}` }}
                    className={styles.s111}
                  >
                    ...
                    {w.type === "form" && (
                      <div className={styles.s120}>
                        [Form Embed:{" "}
                        {forms.find((f: any) => f.id === w.config.formId)
                          ?.name || "Select in layout"}
                        ]
                      </div>
                    )}
                    {w.type === "table" && (
                      <div className={styles.s120}>
                        [Table Listing:{" "}
                        {dataModels.find(
                          (dm: any) => dm.id === w.config.dataModelId,
                        )?.name || "Select in layout"}
                        ]
                      </div>
                    )}
            ...
            fields.length > 0 ? (
              <>
                <DynamicFormRenderer
                  schema={fields}
                  submitLabel="Submit (simulated)"
                  onSubmit={(d) => setLastPayload(d)}
                />
                {lastPayload && (
                  <div className={styles.s122}>
                    <div className={styles.s123}>
                      Captured payload (not saved)
                    </div>
                    <pre className={styles.s124}>
              ...
              <EmptyState
                ...
                description="Link a form to this page (Pages tab) and add fields to preview it."
              />
            )
          ) : (
            <div className={styles.s125}>
              {activePage?.type === "dashboard" ? (
                <BarChart3 size={36} className={styles.s126} />
              ) : (
                <ListIcon size={36} className={styles.s126} />
              )}
              <div className={styles.s26}>
                {activePage?.type === "dashboard" ? "Dashboard" : "List"} page
              </div>
              <div className="text-xs">
                {isInstalledRoute
                  ? "Renders live record data once installed — use “Open Live”."
                  : "Renders live record data at runtime after publish + install."}
              </div>
            </div>
          )}
        </div>
...
function PublishSection({
  ...
  const [scope, setScope] = useState<"ORGANIZATION" | "GLOBAL">(
    app.scope === "GLOBAL" ? "GLOBAL" : "ORGANIZATION",
  );
  const [bump, setBump] = useState<"patch" | "minor" | "major">("patch");
  const [changelog, setChangelog] = useState("");
  const [category, setCategory] = useState(
    (app as any).category || "Operations",
  );
  const [publisher, setPublisher] = useState((app as any).publisher || "");
  const [longDescription, setLongDescription] = useState(
    (app as any).longDescription || "",
  );
  ...
  const fetchReleases = useCallback(async () => {
    try {
      const data = await client.get<any[]>(
        `/builder/modules/${app.id}/releases`,
      );
      setReleases(data);
    } catch {
      /* ignore */
  ...
  const checks = [
    ...
    { label: "App has pages", pass: pages.length > 0 },
    {
      ...
      pass: !!app.description && app.description.length >= 10,
    },
  ...
  return (
    <div>
      <SectionHeader
        ...
        subtitle="Cut an immutable release and deploy it to the App Store"
      />

      {error && <div className={styles.s127}>{error}</div>}
      ...
      <div className={styles.s80}>
        <div className={styles.s128}>
          ...
          >
            {isPublished ? (
              <CheckCircle size={24} className={styles.s130} />
            ) : (
              <Clock size={24} className={styles.s131} />
            )}
          </div>
          <div className="flex-1">
            <div className={styles.s13}>
              {isPublished ? `Published · v${app.version}` : "Draft"}
            </div>
            <div className={styles.s7}>
              {isPublished
                ? `Live on ${app.scope === "GLOBAL" ? "App Store (Global)" : "Organization Only"}`
        ...
        <div className={styles.s32}>
          ...
          <div className="ui-stack-2">
            {checks.map((check, i) => (
              ...
              >
                {check.pass ? (
                  <CheckCircle size={16} className={styles.s130} />
                ) : (
        ...
        <div className={styles.s134}>
          <div>
            <label className="ui-label">
              Version Bump → <span className={styles.s135}>v{nextVersion}</span>
            </label>
            <div className="ui-flex ui-gap-2">
                  ...
                  style={{
                    ...
                    background:
                      bump === b
                        ? "var(--color-primary-bg)"
                        : "var(--color-bg)",
                    fontWeight: bump === b ? 600 : 400,
                    color:
                      bump === b
                        ? "var(--color-primary)"
                        : "var(--color-text-secondary)",
                  }}
                  className={styles.s136}
                >
                  {b}
                </button>
              ))}
          ...
          <div>
            ...
            <div className="ui-flex ui-gap-2">
              {[
                {
                  v: "ORGANIZATION" as const,
                  label: "Organization",
                  icon: Lock,
                },
                { v: "GLOBAL" as const, label: "App Store", icon: Globe },
              ].map((s) => (
                <button
                  key={s.v}
        ...
        <div className="mb-4">
          ...
          <textarea
            ...
            placeholder="Detailed description shown on the App Store listing..."
            style={{ ...inputStyle }}
            className={styles.s138}
          />
        </div>
        <div className={styles.s32}>
      ...
      <div className={styles.s57}>
        ...
        {releases.length === 0 ? (
          <div className={styles.s142}>
            No releases yet. Publish to cut your first release.
          </div>
        ) : (
          <div className="ui-stack-2">
            {releases.map((r: any) => {
                  ...
                  <div className="flex-1">
                    <div className={styles.s72}>
                      ...
                      <span
                        style={{
                          background:
                            r.channel === "GLOBAL" ? "#ede9fe" : "#e0f2fe",
                          color: r.channel === "GLOBAL" ? "#7c3aed" : "#0369a1",
                        }}
                        className={styles.s145}
                      >
                        {r.channel === "GLOBAL" ? "App Store" : "Org"}
                      </span>
                      {isCurrent && (
                        <span className={styles.s146}>CURRENT</span>
                      )}
                      {r.status === "ROLLED_BACK" && (
                        <span className={styles.s147}>SUPERSEDED</span>
                      )}
                    </div>
                    <div className={styles.s148}>
                      {r.changelog || "No changelog"} ·{" "}
                      {new Date(r.publishedAt).toLocaleDateString()}
                      {r.testScore != null ? ` · score ${r.testScore}%` : ""}
                    </div>
                  </div>
                  {!isCurrent && (
                    <button
                    ...
                    >
                      <RotateCcw size={12} />{" "}
                      {rollingBack === r.id ? "Restoring..." : "Rollback"}
                    </button>
                  )}
                </div>
              );
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (rollbackTarget) {
            handleRollback(rollbackTarget.releaseId);
...
export default function AppStudioPage() {
  ...
  const [formBuilder, setFormBuilder] = useState<{
    formId: string;
    pendingLink: boolean;
    pageDraft?: { name: string; slug: string; type: string };
  } | null>(null);
  ...
  const linkComponentToApp = useCallback(
    async (type: "workflow" | "dashboard", refId: string, name: string) => {
      const already = (
        Array.isArray(app?.components) ? app!.components : []
      ).some((c: any) => c.type === type && c.refId === refId);
      if (already) return;
      try {
        await client.post(`/builder/modules/${appId}/components`, {
          type,
  ...
  const handleFormSaved = useCallback(
    async (form: { id: string; name: string }) => {
      const current = formBuilder;
      if (current?.pendingLink) {
        const already = (
          Array.isArray(app?.components) ? app!.components : []
        ).some((c: any) => c.type === "form" && c.refId === form.id);
        if (!already) {
          try {
            await client.post(`/builder/modules/${appId}/components`, {
              type: "form",
      ...
      setFormBuilder((prev) =>
        prev ? { formId: form.id, pendingLink: false } : prev,
      );
      fetchApp();
      fetchForms();
    },
      ...
      <div className={styles.s155}>
        ...
        <div className={styles.s164}>
          {activeSection === "overview" && (
            <OverviewSection app={app} onUpdate={handleUpdate} />
          )}
          {activeSection === "pages" && (
            <PagesSection
              app={app}
          ...
          {activeSection === "forms" && (
            <ComponentListSection
              ...
              onRefresh={fetchApp}
              onBuildNew={() =>
                setFormBuilder({ formId: "new", pendingLink: true })
              }
              onEditItem={(refId) =>
                setFormBuilder({ formId: refId, pendingLink: false })
              }
            />
          )}
          {activeSection === "data-models" && (
            <DataModelsSection app={app} onRefresh={fetchApp} />
          )}
          {activeSection === "workflows" && (
            <ComponentListSection
              ...
              onRefresh={fetchApp}
              onBuildNew={() =>
                setWorkflowBuilder({ workflowId: "new", pendingLink: true })
              }
              onEditItem={(refId) =>
                setWorkflowBuilder({ workflowId: refId, pendingLink: false })
              }
            />
          )}
          {activeSection === "dashboards" && (
            <ComponentListSection
              ...
              onRefresh={fetchApp}
              onBuildNew={() =>
                setDashboardBuilder({ dashboardId: "new", pendingLink: true })
              }
              onEditItem={(refId) =>
                setDashboardBuilder({ dashboardId: refId, pendingLink: false })
              }
            />
          )}
          ...
          {activeSection === "test" && (
            <TestEngineSection app={app} onRefresh={fetchApp} />
          )}
          {activeSection === "publish" && (
            <PublishSection app={app} onRefresh={fetchApp} />
          )}
        </div>
        ...
        {workflowBuilder && (
          <WorkflowEditorWorkspace
            ...
            onSaved={async (wf: { id: string; name: string }) => {
              if (workflowBuilder.pendingLink)
                await linkComponentToApp("workflow", wf.id, wf.name);
              setWorkflowBuilder((prev) =>
                prev ? { workflowId: wf.id, pendingLink: false } : prev,
              );
              fetchApp();
            }}
            onBack={() => {
              setWorkflowBuilder(null);
              fetchApp();
            }}
          />
        )}
        ...
        {dashboardBuilder && (
          <DashboardEditorWorkspace
            ...
            onSaved={async (d: { id: string; name: string }) => {
              if (dashboardBuilder.pendingLink)
                await linkComponentToApp("dashboard", d.id, d.name);
              setDashboardBuilder((prev) =>
                prev ? { dashboardId: d.id, pendingLink: false } : prev,
              );
              fetchApp();
            }}
            onBack={() => {
              setDashboardBuilder(null);
              fetchApp();
            }}
          />
        )}
