...
export default function WorkflowsPage() {
  ...
  const fetchWorkflows = async () => {
    setLoading(true);
    try {
      const data = await client.get<Workflow[] | { data?: Workflow[] }>(
        "/builder/workflows",
      );
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("unerp_page_registry_updated", fetchWorkflows);
  }, [client]);
        ...
        actions={
          <div className="ui-flex ui-gap-2">
            ...
            <button
              ...
              onClick={() => router.push("/builder/erp/workflows/new")}
            >
              <PlusCircle size={15} />
      ...
      {loading ? (
        ...
        <div className={`ui-card ${styles.s8}`}>
          ...
          <p className="ui-text-muted">
            No workflows found. Create your first approval workflow!
          </p>
        </div>
      ) : (
        <div className={styles.s10}>
          {filtered.map((wf) => (
            <div
              key={wf.id}
              className={`ui-card ${styles.s11} ${styles.workflowCard}`}
            >
              <div className={styles.s12}>
                <div className="ui-hstack-2">
                  ...
                  <div>
                    ...
                    {wf.description && (
                      <p className="ui-text-xs-muted m-0">{wf.description}</p>
                    )}
                  </div>
                </div>
                <button
              ...
              <div className={styles.s17}>
                ...
                {wf.docType && <span className={styles.s18}>{wf.docType}</span>}
              </div>
              ...
              <div className="ui-flex ui-gap-1">
                <button
                  ...
                  onClick={() => router.push(`/builder/erp/workflows/${wf.id}`)}
                >
                  <Edit3 size={13} /> <span>Edit</span>
                </button>
                <button
                  className={`ui-btn ui-btn-secondary ${styles.s22}`}
                  onClick={() =>
                    router.push(`/builder/erp/workflows/${wf.id}?preview=true`)
                  }
                >
                  <Eye size={13} />
                </button>
                <button
                  className={`ui-btn ${styles.s23}`}
