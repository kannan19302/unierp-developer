...
function ERPFormsPageContent() {
  ...
  const [sortBy, setSortBy] = useState<"name" | "submissions" | "lastEdited">(
    "name",
  );
  ...
  const loadForms = async () => {
    ...
    try {
      ...
      if (moduleFilter && moduleFilter !== "All Modules")
        query.append("module", moduleFilter);
      ...
      await client.post("/builder/forms", {
        ...
        fields:
          typeof original.fields === "string"
            ? JSON.parse(original.fields)
            : original.fields,
        settings:
          typeof original.settings === "string"
      ...
      <DataTable
        columns={[
          ...
          },
          {
            ...
            header: "Module",
            render: (row: any) => (
              <span className={styles.s13}>{row.module}</span>
            ),
          },
          ...
          },
          {
            ...
            header: "Last Edited",
            render: (row: any) => (
              <span className={styles.s14}>{row.lastEdited}</span>
            ),
          },
          {
            key: "status",
            ...
            render: (row: any) => (
              ...
              >
                <button
                  ...
                  title="Edit Form"
                  onClick={() => router.push(`/builder/erp/forms/${row.id}`)}
                >
                  <Edit3 size={13} />
                </button>
                <button
                ...
                <button
                  ...
                  title="Preview"
                  onClick={() =>
                    router.push(`/builder/erp/forms/${row.id}?preview=true`)
                  }
                >
                  <Eye size={13} />
                </button>
...
export default function ERPFormsPage() {
  return (
    <Suspense
      fallback={<div className={styles.s18}>Loading Form Builder...</div>}
    >
      <ERPFormsPageContent />
    </Suspense>
  );
