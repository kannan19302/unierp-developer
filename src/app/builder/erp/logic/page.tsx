...
const LOGIC_SUB_TABS = [
  {
    id: "rules",
    label: "My Rules",
    href: "/builder/erp/logic?subtab=rules",
    icon: Zap,
  },
  {
    id: "builder",
    label: "Rule Builder",
    href: "/builder/erp/logic?subtab=builder",
    icon: GitBranch,
  },
];
...
function ERPLogicPageContent() {
  const client = useApiClient();
  const router = useRouter();
  const searchParams = useSearchParams();
  const activeTab: "rules" | "builder" =
    searchParams.get("subtab") === "builder" ? "builder" : "rules";
  ...
  const handleToggleStatus = async (rule: any) => {
    ...
    try {
      await client.patch(`/builder/automation-rules/${rule.id}`, {
        status: newStatus,
      });
      fetchRules();
    } catch (e) {
  ...
  return (
    <div className="p-6 ui-stack-5">
      ...
      <div className={styles.s1}>
        {[
          ...
          <div key={stat.label} className={`ui-card ${styles.s2}`}>
            ...
            <span style={{ color: stat.color }} className={styles.s3}>
              {stat.value}
            </span>
          </div>
        ))}
                ...
                <div key={rule.id} className="ui-card p-4">
                  <div className={styles.s8}>
                    ...
                    <div className={styles.s13}>
                      <button
                        className={`ui-btn ui-btn-secondary ${styles.s14}`}
                        onClick={() => handleTestRun(rule.id)}
                      >
                        <Play size={12} />{" "}
                        <span className={styles.s15}>Test</span>
                      </button>
                      {rule.status === "ACTIVE" ? (
                        <button
                  ...
                  <div className={styles.s17}>
                    <div className={styles.s18}>
                      ...
                      <span className="ui-text-xs-muted">
                        Trigger: <strong>{rule.trigger}</strong>
                      </span>
                    </div>
                    <div className={styles.s18}>
                    ...
                    <div className={styles.s20}>
                      ...
                      <span className="ui-text-xs-tertiary">
                        {rule.runs.toLocaleString()} runs · Last: {rule.lastRun}
                      </span>
                    </div>
                  </div>
                </div>
        ...
        <div className={styles.s21}>
          ...
          <div className={`ui-card ${styles.s22}`}>
            <div className="ui-card p-4">
              <div className="mb-4">
                <p className={styles.s23}>Rule Name</p>
                <input
                  type="text"
                  value={ruleName}
              ...
              <div className="mb-4">
                <p className={styles.s23}>Description</p>
                <input
                  type="text"
                  value={ruleDesc}
            ...
            {[
              ...
              <div key={group} className={styles.s25}>
                ...
                {TRIGGER_TYPES.filter((t) => t.group === group).map(
                  (trigger) => (
                    <div
                      key={trigger.id}
                      ...
                      className={`${styles.s27} ${selectedTrigger !== trigger.id ? styles.triggerOption : ""}`}
                    >
          ...
          <div className="ui-card p-4 ui-stack-3">
            ...
            <div className="ui-form-group">
              ...
              <input
                ...
                value={ruleName}
                onChange={(e) => setRuleName(e.target.value)}
                placeholder="e.g. Auto-Assign Invoice on Submit"
              />
            </div>
            ...
            <div className={styles.s35}>
              ...
              <div className={styles.s37}>
                <select className="ui-input text-xs">
                ...
                <button
                  ...
                  className={`ui-btn ui-btn-secondary ${styles.s38}`}
                >
                  <Trash2 size={12} />
                </button>
              </div>
              <button
                ...
                className={`ui-btn ui-btn-secondary ${styles.s39}`}
              >
                <PlusCircle size={12} />
            ...
            <div className={styles.s40}>
                ...
                return (
                  <div key={i} className={styles.s42}>
                    <div className={styles.s43}>
                      {action && (
                        <action.icon size={13} className={styles.s44} />
                      )}
                    </div>
                    <select
                      className={`ui-input ${styles.s45}`}
                      value={actionId}
                      onChange={(e) => {
                    ...
                    <button
                      ...
                      onClick={() =>
                        setSelectedActions(
                          selectedActions.filter((_, idx) => idx !== i),
                        )
                      }
                    >
                      <Trash2 size={12} />
              ...
              <button
                className={`ui-btn ui-btn-secondary ${styles.s47}`}
                onClick={() =>
                  setSelectedActions([...selectedActions, "send_notification"])
                }
              >
                <PlusCircle size={12} />
          ...
          <div className={`ui-card ${styles.s22}`}>
            <p className={styles.s48}>Action Library</p>
            ...
            <div className={styles.s52}>
              ...
              {[
                ...
                <label key={toggle.label} className={styles.s54}>
                  ...
                  >
                    <div
                      style={{ left: toggle.checked ? "18px" : "2px" }}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (deleteTarget) {
            executeDeleteRule(deleteTarget);
...
export default function ERPLogicPage() {
  return (
    <Suspense fallback={<div className="p-6">Loading Business Logic…</div>}>
      <ERPLogicPageContent />
    </Suspense>
  );
}
