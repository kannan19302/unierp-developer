...
function CreateAppModal({
  ...
  const handleSubmit = async () => {
    ...
    setError("");
    try {
      const data = await client.post<{ id: string }>("/builder/modules", {
        name,
        slug,
        description,
        icon,
...
function AppCard({
  ...
  return (
    ...
    >
      {/* Color bar */}
      <div
        style={{ background: app.color || "var(--color-primary)" }}
      ...
      <div className="p-5">
        ...
        {/* Description */}
        {app.description && <p className={styles.s30}>{app.description}</p>}
...
function ERPBuilderPageContent() {
  ...
  const stats = {
    ...
    tested: apps.filter((a) => {
      const tr =
        a.testResults &&
        typeof a.testResults === "object" &&
        !Array.isArray(a.testResults)
          ? (a.testResults as any)
          : {};
    ...
    <div className={styles.s36}>
            ...
            style={{
              border:
                filter === f
                  ...
                  : "1px solid var(--color-border)",
              background:
                filter === f ? "var(--color-primary-bg)" : "var(--color-bg)",
              color:
                filter === f
                  ? "var(--color-primary)"
                  : "var(--color-text-secondary)",
        ...
        <div className={styles.s53}>
          ...
          <p className={styles.s56}>
            {search || filter !== "all"
              ...
              : "Create your first custom ERP application to get started."}
          </p>
          {!search && filter === "all" && (
            <button
              onClick={() => setShowCreateModal(true)}
      ...
      <CreateAppModal
        ...
        onSuccess={(id) => {
          ...
          router.push(`/builder/erp/apps/${id}`);
        }}
      />
