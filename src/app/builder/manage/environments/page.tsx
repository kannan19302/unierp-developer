...
export default function EnvironmentsPage() {
  ...
  const fetchEnvironments = useCallback(
    async (appId: string) => {
      ...
      try {
        setEnvironments(
          await client.get(`/builder/governance/modules/${appId}/environments`),
        );
      } catch (e) {
        console.error(e);
      } finally {
  ...
  const executePromotion = async (target: "staging" | "production") => {
    try {
      const endpoint =
        target === "staging" ? "promote-staging" : "promote-production";
      await client.post(
        `/builder/governance/modules/${selectedAppId}/${endpoint}`,
      );
      fetchEnvironments(selectedAppId);
    } catch (e) {
      console.error(e);
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "Deployed At",
      render: (row: any) =>
        row.promotedAt ? new Date(row.promotedAt).toLocaleString() : "—",
    },
    { key: "promotedBy", header: "Deployed By" },
    {
      ...
      render: (row: any) => {
        if (row.environment === "DRAFT") {
          return (
            <button
              ...
              onClick={() =>
                setActionConfirm({ type: "staging", name: "Staging" })
              }
            >
        ...
        if (row.environment === "STAGING") {
          return (
            <button
              ...
              onClick={() =>
                setActionConfirm({ type: "production", name: "Production" })
              }
            >
        ...
        return (
          <span className="ui-text-xs-soft">Latest Deployment Staged</span>
        );
      },
    },
  ];
