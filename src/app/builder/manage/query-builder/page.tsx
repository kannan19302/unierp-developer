...
export default function VisualQueryBuilderPage() {
  ...
  const [queries, setQueries] = useState<any[]>([
    {
      ...
      query: "SELECT * FROM Customer WHERE status = 'ACTIVE' LIMIT 100;",
    },
    {
      id: "q2",
      name: "High Value Orders",
      query: "SELECT * FROM SalesOrder WHERE totalAmount > 5000 LIMIT 50;",
    },
  ]);
  ...
  const handleTestRun = async () => {
    ...
    try {
      const data = await client.post<any>("/builder/governance/query/run", {
          ...
          <div className={styles.s3}>
            {selectedQuery && (
              <div className="ui-card p-5">
                ...
                >
                  <Play size={14} />{" "}
                  {running ? "Executing check..." : "Test Run SQL"}
                </button>
              </div>
            )}
            ...
            {consoleOutput && (
              <div className="ui-card p-4">
                ...
                <pre className={styles.s6}>{consoleOutput}</pre>
              </div>
            )}
          </div>
        </div>
