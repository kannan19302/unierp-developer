...
export default function NativeExportPage() {
  ...
  const handleBuildTrigger = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await client.post("/builder/native-builds/trigger", {
        version,
        platform,
      });
      fetchBuilds();
  ...
  const columns = [
    ...
    { key: "platform", header: "Platform" },
    {
      ...
      render: (row: any) => (
        <span
          className={`ui-badge ${row.status === "COMPLETED" ? "ui-badge-success" : "ui-badge-warning"}`}
        >
          {row.status}
        </span>
      ),
      ...
      <div className={styles.s4}>
        ...
        <div className="ui-card p-5">
          ...
          <form onSubmit={handleBuildTrigger} className="ui-stack-4">
            ...
            >
              <Download size={12} />{" "}
              {loading ? "Compiling mobile shell..." : "Trigger Build"}
            </button>
          </form>
        </div>
        ...
        <div className="ui-card p-5">
          <div className="ui-flex-between mb-4">
            <h4 className={styles.s6}>
              Compilation Log Summary (v{logView.version})
            </h4>
            <button className="ui-btn" onClick={() => setLogView(null)}>
              Close Logs
            </button>
          </div>
