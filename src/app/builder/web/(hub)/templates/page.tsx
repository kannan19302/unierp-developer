...
function WebTemplatesPageContent() {
  const {
    ...
    updateItem,
    deleteItem,
  } = useBuilderData<any>("web-templates", []);
  ...
  return (
    <div className="p-6 ui-stack-5">
      ...
      <div className={styles.s1}>
        {/* Create New */}
        <div
          className={`ui-card ${styles.s2} ${styles.createCard}`}
          onClick={() => {
            setEditingItem(null);
            setIsModalOpen(true);
          }}
        ...
        {TEMPLATES_DB.map((template: any) => {
          ...
          return (
            <div
              key={template.id}
              className={`ui-card ${styles.s6} ${styles.templateCard}`}
            >
              {/* Preview */}
              <div
                style={{
