...
const MENUS_SUB_TABS = [
  {
    id: "list",
    label: "All Menus",
    href: "/builder/web/menus?subtab=list",
    icon: Navigation,
  },
  {
    id: "editor",
    label: "Menu Editor",
    href: "/builder/web/menus?subtab=editor",
    icon: Edit3,
  },
];

function WebMenusPageContent() {
  const {
    data: MENUS_DB,
    createItem,
    updateItem,
    deleteItem,
  } = useBuilderData("web-menus", []);
    ...
    <div className="p-6 ui-stack-5">
      ...
      {activeTab === "list" && (
        <div className={styles.s3}>
          {/* Create new card */}
          <div
            className={`ui-card ${styles.s4} ${styles.createCard}`}
            onClick={() => {
              setEditingItem(null);
              setIsModalOpen(true);
            }}
          ...
          {MENUS_DB.map((menu: any) => {
            ...
            const previewItems = Array.isArray(menu.items)
              ? menu.items.slice(0, 4).map((i: any) => i.label || "Item")
              : ["Home", "Products", "Contact"];
            return (
              <div
                key={menu.id}
                className={`ui-card ${styles.s7} ${styles.menuCard}`}
                onClick={() => {
                  setEditingItem(menu);
                  router.push("/builder/web/menus?subtab=editor");
                }}
              >
                  ...
                  <div className="ui-hstack-2">
                    <div className={styles.s9}>
                      {menu.location === "MOBILE" ? (
                        <Smartphone size={18} className={styles.s5} />
                      ) : (
                        <Monitor size={18} className={styles.s5} />
                      )}
                    </div>
                    <div>
                      <p className={styles.s10}>{menu.name}</p>
                      <p className="ui-text-xs-muted m-0">
                        {menu.location} · {itemCount} items
                      </p>
                    </div>
                  </div>
                  <span
                ...
                <div className={styles.s14}>
                  <button
                    className={`ui-btn ui-btn-secondary ${styles.s15}`}
                    onClick={(e) => {
                      e.stopPropagation();
                      setEditingItem(menu);
                      router.push("/builder/web/menus?subtab=editor");
      ...
      {activeTab === "editor" && (
        <div className={styles.s18}>
          ...
          <div className={`ui-card ${styles.s19}`}>
            <div className={styles.s8}>
              <p className={styles.s20}>Menu Items</p>
              <button
                onClick={() => {
                  /* Edit menu item */
                }}
            ...
            <div className="ui-stack-1">
              {(editingItem?.items || []).map((item: any) => (
                <div
                  ...
                  className={`${styles.s22} ${styles.menuItem} ${selectedItem === item.id ? styles.menuItemSelected : ""}`}
                >
                  <GripVertical size={12} className={styles.s23} />
                  {item.children?.length > 0 && (
                    <ChevronRight size={12} className={styles.s24} />
                  )}
                  <span
                    style={{
                      color:
          ...
          <div className="ui-card p-4 ui-stack-3">
            <div className={styles.s27}>
              <h3 className={styles.s28}>
                {editingItem?.name || "Main Navigation"}
              </h3>
              <div className="ui-flex ui-gap-2">
            ...
            <div className={styles.s29}>
              ...
              <div className={styles.s34}>
                ...
                <div className={styles.s38}>
                  {["Home", "Products", "Pricing", "Blog", "Contact"].map(
                    (item, i) => (
                      <span
                        key={item}
                        style={{
          ...
          <div className={`ui-card ${styles.s19}`}>
            <p className={styles.s42}>Item Properties</p>
            {selectedItem ? (
              <div className="ui-stack-3">
                ...
                <button
                  ...
                  className={`ui-btn ui-btn-primary ${styles.s47}`}
                >
                  <CheckCircle size={13} />
                  <span>Save Item</span>
                </button>
      ...
      <GenericBuilderModal
        ...
        fields={[
          ...
          {
            name: "location",
            label: "Location (HEADER/FOOTER/MOBILE)",
            type: "text",
            required: true,
          },
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (deleteTarget) {
            executeDeleteMenu(deleteTarget);
...
export default function WebMenusPage() {
  return (
    <Suspense fallback={<div className="p-6">Loading Menu Builder…</div>}>
      <WebMenusPageContent />
    </Suspense>
  );
}
